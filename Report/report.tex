...
实验中，首先对核磁共振系统进行调节，确保永磁体磁场均匀性满足要求，并利用自由感应衰减（Free Induction Decay， FID）信号，测量一标准样品的拉莫尔频率$f_0$，完成仪器检定。随后，对标准油样品和硫酸铜溶液样品分别测定$90^\circ$脉冲和$180^\circ$脉冲的脉宽$P_1, P_2$，并通过测量选择适当重复采样等待时间$T_W$。

在测定$P_1, P_2, T_W$后，使用Carr-Purcel-Meiboom-Gill（CPMG）序列测量横向弛豫时间$T_2$，并使用反转恢复（Inversion Recovery, IR）序列测量纵向弛豫时间$T_1$。

最后，采用自旋回波成像序列，实现样品内部结构的空间分辨成像：通过调节梯度磁场实现空间编码，并重建样品内部结构图像；调整回波时间$T_E$和重复时间$T_R$，实现不同物质的加权成像并进行对比。
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{../Data/pre-experiment/Oil_T2_final_analysis_plot.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../Data/pre-experiment/SLN_T2_final_analysis_plot.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../Data/pre-experiment/Oil_T1_final_analysis_plot.png}
